...
Each package MUST test its executable SSH updater and local-only Ansible
envelope against this standard, including its singleton or joined-inventory
adapter. Tests and implementation belong in the owning package; this workspace
contains the contract. Package-specific policy and migration variants MUST be
documented and tested to reject broader ownership matching. Package integration
tests MUST prove workflow ordering and inventory behavior, and dependency checks
MUST verify direct immutable compute-library pins in every supported color.
